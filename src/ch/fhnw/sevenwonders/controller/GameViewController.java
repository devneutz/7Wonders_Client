...
public class GameViewController implements Initializable {
	...
	/**
	 * 
	 * @author lucas rueesch Diese Methode prüft ob die Karten zum Bau verwendet
	 *         werden können oder nicht. Wenn Ja ändert sich der in eine HBox
	 *         gewrappte Imageview von rot auf grün
	 * 
	 */
	...
	public void handleUmmunzenButton(ActionEvent event) {
		// Deaktivieren s�mtlicher Interaktionsm�glichkeiten des Spielers - solange
		// bis eine Nachricht vom Server zur�ckkommt.
		RessourceVerwendenButton.setDisable(true);
		UmmunzenButton.setDisable(true);
		ZumBauVerwendenButton.setDisable(true);
	...
	public void handleRessourceVerwendenButton(ActionEvent event) {
		...
		// Zusammenstellen der Nachricht an den Server. Diese beinhaltet die Aktion, die
		// vom Spieler durchgef�hrt werden will.
	...
	public void setupListener(Scene inScene) {
		this.model.getLastReceivedMessage().addListener((observable, oldvalue, newValue) -> {
			...
			// Theoretisch k�nnte hier auch ein Broadcast kommen, welcher dem Client
			...
			if (newValue instanceof ServerGameMessage) {
				...
				// Setzen des Spielers, welcher vom Server zur�ckgegeben wird. Verhindert eine
				...
				case ActionNotAvailable:
					// TODO Alles wieder aktivieren f�r eine n�chste Auswahl? D�rfte gar nie
					// der Fall sein. Aktuell ignorieren
					throw new IllegalArgumentException("Aktion nicht m�glich");
				case NewRound:
					setUpCards();
					// TODO Alles wieder aktivieren, eine neue Runde hat begonnen. Alle ben�tigten
