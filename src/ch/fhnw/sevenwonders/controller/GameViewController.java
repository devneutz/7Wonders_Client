...
public class GameViewController {
		...
		Platform.runLater(new Runnable() {
			public void run() {
				...
				for (int x = 0; x < model.getPlayer().getCardStack().size(); x++) {
					...
					// Existiert die Ressource nicht (Bild fehlt o.ö.) wird es ausgegeben
					if(tmpResource == null) {
	...
	/**
	 * Wird ausgef�hrt, wenn eine Ressource gespielt werden soll
	 * @param event
	 */
	public void handleRessourceVerwendenButton(ActionEvent event) {
		...
		// Ruft Methode auf, um die Ressourcen ebenfalls in der Spieler�bersicht
		// hinzuzufuegen.
	...
	/**
	 * Wird ausgef�hrt, wenn eine Karte f�r den Bau eines Weltwunders gebraucht werden soll
	 * @param event
	 */
	...
	/**
	 * Toggle-Event f�r die Karten des Spielers 1
	 * @param event
	 */
	...
	/**
	 * Toggle-Event f�r die Karten des Spielers 2
	 * @param event
	 */
	...
	/***
	 * Toggle-Event f�r die Karten des Spielers 3
	 * @param event
	 */
	...
	/**
	 * Toggle-Event f�r die Karten des Spielers 4
	 * @param event
	 */
	...
	/**
	 * Toggle-Event f�r die Karten des Spielers 5
	 * @param event
	 */
	public void handlePlayer5Label(MouseEvent event) {
		...
		for (ICard c : ((IPlayer) player5ResourceBox.getUserData()).getCards()) {
			...
			if (tmpResource != null) {
				img.setImage(new Image(tmpResource.toExternalForm()));
				Player5ResVBox.getChildren().add(img);
			} else {
				System.out.println(c.getImageName());
			}
		}
	...
	/**
	 * Toggle-Event f�r die Karten des Spielers 6
	 * @param event
	 */
	...
	/**
	 * Registrieren der Listener f�r UI Aktionen bei einkommender Nachricht
	 * 
	 * @param inScene
	 */
	...
	/**
	 * @author lucas Rüesch: Event wird beim Auswählen einer Karte ausgeführt. 
	 * @param inEvent
	 */
	@FXML
	 ...
	 * Changelistener, welcher auf �nderungen der einkommenenden Nachrichten reagiert
	 */
	private ChangeListener<Message> changeListener = new ChangeListener<Message>() {
		...
		public void changed(ObservableValue observable, Message oldValue, Message newValue) {
			...
			if (newValue instanceof ServerGameMessage) {
				...
				// Idee falls genug Zeit: Bei einem Success eine Meldung zur�ckgeben, dass auf
				...
				case NewRound:
					// Aktualisieren s�mtlicher Variablen und UI Komponenten
					WaitLabel.setVisible(false);
					updateCardUi();
					updateOpponentsUi();
					break;
			...
			// Handelt es sich um den letzten Zug, kommt vom Server eine Auswertungs-Nachricht zur�ck
			if(newValue instanceof ServerEvaluationMessage) {
				// Listener entfernen damit nichts mehr ausgef�hrt wird im Hintergrund
				...
				// Weiterleiten auf die Auswertungs-�bersicht
				Platform.runLater(new Runnable() {
					public void run() {
						try {
